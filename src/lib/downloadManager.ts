...
// 使用 Electron API 保存檔案
const saveWithElectron = async (blob: Blob, filename: string): Promise<boolean> => {
  ...
  try {
    ...
    const base64Promise = new Promise<string>((resolve, reject) => {
      reader.onload = () => {
        if (typeof reader.result === 'string') {
          // 移除前綴 "data:image/jpeg;base64," 只保留 base64 內容
    ...
    console.error('使用 Electron 保存檔案失敗:', error);
    return false;
  }
};

// 修改現有的下載函數以支持 Electron
export const downloadFile = async (blob: Blob, filename: string): Promise<boolean> => {
  // 先嘗試使用 Electron API
  if (isElectron()) {
    const success = await saveWithElectron(blob, filename);
    if (success) {
      return true;
    }
  }
  
  // 如果 Electron 保存失敗或不在 Electron 環境，使用瀏覽器 API
  try {
    // ... existing browser download code ...
  } catch (error) {
